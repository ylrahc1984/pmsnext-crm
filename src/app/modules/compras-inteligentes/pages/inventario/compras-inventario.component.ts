...
export class ComprasInventarioComponent implements OnInit {
  ...
  readonly filtrosAplicados = signal<{ periodo: string; almacen: string; proveedor: string } | null>(null);
  ...
  private cargarCatalogos(): void {
    ...
    forkJoin({ almacenes: this.almacenService.getAlmacenes() })
      .pipe(finalize(() => this.catalogosLoading.set(false)), takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: ({ almacenes }) => {
  ...
  private ejecutarConsulta(pageNumber: number, pageSize: number): void {
    ...
    this.consultaSubscription = this.reportesService.getInventarioHistorico({
